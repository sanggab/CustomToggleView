...
ToggleModel은 기본적인 default value들을 가지고 있는데 만약 기본 style로 사용할 경우 custom Modifier을 구현안해도 됩니다.
...
Switch의 isOn의 상태에 따라 Color을 바꿀 수 있습니다.
...
Switch에 Color가 아닌 content closure에 구현한 View로 대체할 수 있습니다.
...
knob의 Color를 isOn State에 따라 바꿉니다.
...
기본적으로 Padding을 적용안시키면 Switch의 Height이랑 같아서 Padding을 주고 싶을 떄 사용하면 됩니다.   
사용법은 padding() modifier랑 같습니다.

**Method:**

* **`knobPadding(_ edges: Edge.Set = .all, _ length: CGFloat = .zero) -> CustomToggleView`**   
knob에 padding을 적용시킵니다.   
length을 입력 안할경우, 0으로 들어갑니다.
...
isOn의 state가 true 일 때의 knob padding을 적용시킵니다.   
length을 입력 안할경우, 0으로 들어갑니다.   
...
isOn의 state가 false 일 때의 knob padding을 적용시킵니다.   
length을 입력 안할경우, 0으로 들어갑니다.   
...
Knob 위에 CustomView를 적용시키고 싶을 때 사용합니다.
